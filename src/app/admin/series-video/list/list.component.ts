...
export class ListComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute, private seriesVideoService: SeriesVideoService, private toastr: ToastrService) { }
  ...
  seriesVideoList(page) {
    let search = ($('.table_search').val()) ? ($('.table_search').val()) : '';
    this.seriesVideoService.seriesVideoList(page, this.series_id, search).subscribe(
      result => {
  ...
  handlePageChange(page){
    ...
    this.seriesVideoList(page);
  }
  deleteSeriesVideo(id: any) {
    if (confirm("Are you sure to delete ?")) {
      this.seriesVideoService.deleteSeriesVideo(id).subscribe(
        result => {
          if (result.success == true) {
  ...
  createVideoSliceRecordong(videoId, folderId) {
    ...
    this.seriesVideoService.createVideoSliceRecordong(postData).subscribe(result => {
      if (result.success) {
  ...
  if (this.notationFile) {
      ...
      this.seriesVideoService.addNotation(formData).subscribe(result => {
        if (result.success) {
  ...
  updateOrderOfList(){
    ...
    formData.append('model', 'series_video');
    formData.append('series_id', this.series_id);
    this.seriesVideoService.updateOrder(formData).subscribe(
      result => {
        if (result.success == true) {
          this.seriesVideoList(this.currentPage)
  ...
  getCurrentList(){
    this.seriesVideoService.getCurrentDragList(this.currentPage,this.series_id,'series_video').subscribe(
      result => {
        if (result.success == true) {
