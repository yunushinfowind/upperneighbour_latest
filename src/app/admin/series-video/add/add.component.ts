...
import { SeriesServiceService } from '../../series/series-service.service';
...
export class AddComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private activatedRoute: ActivatedRoute, private toastr: ToastrService, private seriesVideoService: SeriesVideoService, private router: Router, private routineSer: SeriesServiceService) {
  ...
  ngOnInit(): void {
    ...
    this.getTeacherList()
    this.getRoutineDetail(this.series_id);
  }
  ...
  getRoutineDetail(id) {
    this.routineSer.routineDetail(id).subscribe(result => {
      if (result.success) {
        this.model.series_name = result.data.series_name;
      }
    })
  }
  ...
  onSubmit() {
    ...
    if (formLength > 0) {
      ...
      if (!this.productForm.invalid && (this.sizeSum < 200)) {
        ...
        this.seriesVideoService.addSeriesVideo(formData).subscribe(result => {
          if (result.success) {
            this.showLoader = false;
            this.toastr.success(result.message);
            $('#loader_submit').hide();
            $('#submit_button').attr('disabled', 'false');
            this.router.navigate(['/admin/series-video/list', this.series_id, this.user_id]);
